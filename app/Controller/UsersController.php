...
class UsersController extends AppController {
	...
	public function admin_view($id = null) {
		...
		$department = $this->Department->find('first', array(
			...
			'conditions' => array('Department.status' => '1', 'Department.id' => $user['Profile']['department_id']),
			'recursive'  => 0
	    ));
	    $city = $this->City->find('first', array(
			'fields'     => array('City.title'),
			'conditions' => array('City.status' => '1', 'City.id' => $user['Profile']['city_id']),
			'recursive'  => 0
	    ));
	    $this->set( compact('city', 'department', 'user') );
	}
	...
	public function admin_city_to_departmentAjax(){
	...
	public function login() {
	    ...
	    if ($this->request->is('post')) {
	        if ($this->Auth->login()) {
				...
				$current        = date("Y-m-d H:i:s");
				$this->User->saveField('last_login',$current);
				$this->User->saveField('ip_address', $_SERVER['REMOTE_ADDR']);
    			$group_id = $this->Auth->user('group_id');
    			if( $group_id != 1 && $group_id != 2 ){
    				if( $this->Auth->user('status') == "0" ){
            ...
            if($this->User->validates())	{
                ...
                if ($this->User->saveAll($this->request->data)) {
					...
					$Email->template('registration_admin', 'default')
					    ...
					    ->subject('ICargoBox.com - New Registration')
					    ->viewVars(
						    	array(
					...
					$Email->template('activation_link_member', 'default')
					    ...
					    ->subject('ICargoBox.com - Account Activation Link')
					    ->viewVars(
						    	array(
									'fname'          => $this->request->data['Profile']['fname'],
									'lname'          => $this->request->data['Profile']['lname'],
